...
//-----------------------------------------------------------------------------------
// ＳＶ配列
var dummy_subclass =  new Array("('','00',true,true)");
var Array_subclass =  new Array("('','00',true,true)");
// ----------------
//	DR_SV_SEARCH
// ----------------
function news_dept_subclass_search(sw) {
	if ( $submit_sw == "1" ){ return; }				// 送信中は、何もせずに抜ける。
  if ( document.form1.select_deptclass.value != "00000" ){
	...
	ApWindow.document.form1.action="Xykjlj_newsSubclassJ.jsp";
	...
	document.form1.select_subclass.disabled = false;
	focus();
	$submit_sw = "1";								// 送信中 SWITCH ON
  }else{
	clear_subclass(document.form1);
	document.form1.select_subclass.disabled = true;
//	alert("ＤＥＰＴ／ＣＬＡＳＳを選択してください。");
  }
}
// ****************************
// * ten_COMBO_BOX 書換
// ****************************
function pop_subclass(inForm) {
  var selectedArray = Array_subclass;
  var x = 0;
  var Array_name = "";
  while (selectedArray.length < inForm.select_subclass.options.length) {
   inForm.select_subclass.options[(inForm.select_subclass.options.length - 1)] = null;
  }

  x = Array_subclass.length;
  for (var i=0; i < x; i++) {
	inForm.select_subclass.options[i]= null;
  }

  Array_name = eval("Array_subclass");
  for (var i=0; i < x; i++) {
	 eval("inForm.select_subclass.options[i]=" + "new Option" + eval(Array_name)[i]);
  }

  inForm.select_subclass.disabled = false;
//	inForm.submit_button0.disabled = false; //検索ボタンを有効
}
// ****************************
// * ten_COMBO_BOX クリア
// ****************************
function clear_subclass(inForm) {
  var selectedArray = dummy_subclass;
  var x = 0;
  var Array_name = "";
  while (selectedArray.length < inForm.select_subclass.options.length) {
   inForm.select_subclass.options[(inForm.select_subclass.options.length - 1)] = null;
  ...
  x = dummy_subclass.length;
  for (var i=0; i < x; i++) {
	inForm.select_subclass.options[i]= null;
  }

  Array_name = eval("dummy_subclass");
  for (var i=0; i < x; i++) {
	 eval("inForm.select_subclass.options[i]=" + "new Option" + eval(Array_name)[i]);
  }
  inForm.select_subclass.disabled = true;
}
// -->
