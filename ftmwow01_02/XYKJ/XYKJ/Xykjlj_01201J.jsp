...
<%
	//フォームデータ解析
	pageParameters = new Xykjlj_01201_Params(request);
	Xykjlj_01201_Params param = (Xykjlj_01201_Params)pageParameters;
	
	//テーブル作成
	Xykj0120Renewday renewdayTable = new Xykj0120Renewday(param);
	renewdayTable.setMinLines(1);

	//テーブル作成 2003/12/18 miura: checkTable2削除
	Xykj0120CheckTable checkTable = new Xykj0120CheckTable(param);
	checkTable.setMinLines(1);
	
	//テーブル作成
	Xykj0120DetailTable detailTable = new Xykj0120DetailTable(param);
...
// *---------------------------------------------------------------------
// * システム　　名称 : 出版物管理システム
// * 画面名称		  : DEPT-CLASS-SUBCLASS-発行形態　別　検索（Xykjlj_01201J.jsp）
...
<!--
$help_path		   = "<%= XysfljGenericRules.CONT_PATH_XYKJ%>help/Xykjlj_01201.htm";
-->
</SCRIPT>
<SCRIPT language="JavaScript" src="<%= XysfljGenericRules.CONT_PATH_XYKJ%>common/common.js"></SCRIPT>
<SCRIPT language="JavaScript" src="<%= XysfljGenericRules.CONT_PATH_XYKJ%>common/publishcom_brand.js"></SCRIPT>
<SCRIPT language="JavaScript" src="<%= XysfljGenericRules.CONT_PATH_XYKJ%>common/publishbrand_saledate.js"></SCRIPT>
...
function getKEYCODE(e){
	...
	function initial(){
		document.title = G_name( "Sxykj01201" );	// *** 画面名称取得 ***
		resizeTo(1024,740);		// 画面初期表示リサイズ
		focus();				// 自画面へフォーカス
		self.moveTo(0,0);		// Window 位置を0,0へ移動
		if(document.form1.CommandType.value == "<%= Xykjlj_01201_Params.COMMAND_REFERENCE %>"){
...
// 2003/12/18 miura:ポップアップメッセージを削除
	...
	function download_chk(){
		...
		document.form1.CommandType.value = "<%= Xykjlj_01201_Params.COMMAND_DOWNLOAD %>";
		document.form1.method = "POST";
		document.form1.action = "<%= XysfljGenericRules.DOWNLOAD_PATH_XYKJ%>Xykjlj_01201Jd";
...
// --------------------------------------
	function button1_func(){
		...
		document.form1.CommandType.value = "<%= Xykjlj_01201_Params.COMMAND_REFERENCE %>";
...
<FORM method="post" name="form1" target="_top" action="Xykjlj_01201J.jsp">
<%= param.createTagOfStatus()%>
<%= param.createTagOfCommandType()%>
<SCRIPT language="JavaScript">
<!--
	Header("Sxykj01201","",975,0,0,0,0,1);
//			|			|	| | | | |
//			|			|	| | | | +---> 日付時刻 SW
//			|			|	| | | +-----> HELP SW
//			|			|	| | +-------> 印刷 SW
...
<TBODY><TR>
	...
	<TD>出版社</TD>
	<TD><%= (param.getControl(XykjljPUBLISHCOMCombo.NAME)).createTag()%></TD>
	<TD colspan="2">発売日
	<%= (param.getControl(XykjljPUBLISHSALEDATECombo.NAME)).createTag()%></TD>
	<TD></TD>
</TR><TR>
	<TD>又は</TD>
	<TD></TD>
	<TD>銘柄</TD>
	<TD colspan="3"><%= (param.getControl(XykjljPUBLISHBRANDCombo.NAME)).createTag()%></TD>
	<TD>
...
<INPUT type ="hidden" name="screenId" value="SXYKJ01201" >
