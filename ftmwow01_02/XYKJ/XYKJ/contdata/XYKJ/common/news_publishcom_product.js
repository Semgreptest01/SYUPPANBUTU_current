...
//-----------------------------------------------------------------------------------
// ＳＶ配列
var dummy_product =  new Array("('','000000',true,true)");
var Array_product =  new Array("('','000000',true,true)");
// ----------------
//	DR_SV_SEARCH
// ----------------
function news_publishcom_product_search(sw) {
	if ( $submit_sw == "1" ){ return; }				// 送信中は、何もせずに抜ける。
  if ( document.form1.select_publishcom.value != "000000" ){
	...
	ApWindow.document.form1.action="Xykjlj_newsPublishProductJ.jsp";
	ApWindow.document.form1.publishcom_cd.value = document.form1.select_publishcom.value;
	ApWindow.document.form1.blank_sw.value = sw;
	ApWindow.document.form1.method = "POST";
	ApWindow.document.form1.submit();
	document.form1.select_product.disabled = false;
	focus();
	$submit_sw = "1";								// 送信中 SWITCH ON
  }else{
	clear_product(document.form1);
...
// * ten_COMBO_BOX 書換
// ****************************
function pop_product(inForm) {
  var selectedArray = Array_product;
  var x = 0;
  var Array_name = "";
  while (selectedArray.length < inForm.select_product.options.length) {
   inForm.select_product.options[(inForm.select_product.options.length - 1)] = null;
  }

  x = Array_product.length;
  for (var i=0; i < x; i++) {
	inForm.select_product.options[i]= null;
  }

  Array_name = eval("Array_product");
  for (var i=0; i < x; i++) {
	 eval("inForm.select_product.options[i]=" + "new Option" + eval(Array_name)[i]);
  }
	dataNull();
  inForm.select_product.disabled = false;
//	inForm.submit_button0.disabled = false; //検索ボタンを有効
}
// ****************************
// * ten_COMBO_BOX クリア
// ****************************
function clear_product(inForm) {
  var selectedArray = dummy_product;
  var x = 0;
  var Array_name = "";
  while (selectedArray.length < inForm.select_product.options.length) {
   inForm.select_product.options[(inForm.select_product.options.length - 1)] = null;
  ...
  x = dummy_product.length;
  for (var i=0; i < x; i++) {
	inForm.select_product.options[i]= null;
  }

  Array_name = eval("dummy_product");
  for (var i=0; i < x; i++) {
	 eval("inForm.select_product.options[i]=" + "new Option" + eval(Array_name)[i]);
  }
  inForm.select_product.disabled = true;
}
// -->
